  ...
  function action_after_insert($post_array, $approval_id, $header_id): bool
  {
   ...
   function post_approval_action_event($item){
    ...
    if(in_array($next_status, $max_user_approval_status_ids)){
      ...
      $this->create_user_password_reset_token($user_id);
      // Send an email
      ...
      $this->log_new_user_email($user_id, $user_full_name, $email, $token_data);
    }else{
   ...
   private function create_user_password_reset_token($user_id){
    $validityPeriod = $this->config->item('user_token_expiration_time');
...
function user_role_ids_with_expiry_dates($user_id){
  ...
  public function get_user_info($user_id):array{
    ...
    $this->read_db->select(array('user_password_reset_token','user_is_switchable'));
...
public function log_new_user_email($user_id, $user_full_name, $email, $token_data){
      $this->load->model('email_template_model');

      // Get user token
      $this->read_db->where(array('user_id' => $email));
      $this->read_db->get('user');

      $tags['user'] =  $user_full_name;
      $tags['email'] = $email;
      $tags['system_name'] = $this->db->get_where('setting', array('type' => 'system_name'))->row()->description;
