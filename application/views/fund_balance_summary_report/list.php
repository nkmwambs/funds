    ...
    <div class = 'form-group col-xs-3 <?=!$this->session->system_admin ? 'hidden_elem' : '';?>'>
        ...
        <div class = 'col-xs-2'>
            <select class = 'form-control select2' id = 'report_category'>
                ...
                <option value="month_cash_balance"><?=get_phrase('month_cash_balance');?></option>
            </select>
        </div>
...
<script>
   ...
   $("#datatable_filter").html(search_box());
    ...
    function triggerInit(elem, report_category){
        ...
        let columns_url = "<?=base_url();?><?=$this->controller;?>/fund_columns/" + report_category;
        let data_url = "<?=base_url();?><?=$this->controller;?>/fund_show_list/" + report_category ;
