...
//$voucher_account_name = $voucher_header_info['voucher_type_account_name'];
//$voucher_effect_name = $voucher_header_info['voucher_type_effect_name'];
//$is_chq_reference = $voucher_header_info['voucher_type_is_cheque_referenced'];



print_r($voucher_header_info);


//print_r($this->voucher_model->unapproved_month_vouchers(14, '2022-08-04'));
?>
                ...
                <div class='form-group'>
                    ...
                    <div class='col-xs-2'>
                        <select class='form-control required ' id='office' name='fk_office_id' readonly>
                            ...
                            <option value="<?= $voucher_header_info['office_id']; ?>"><?= $voucher_header_info['office_code']; ?></option>
                        </select>
                    </div>

                    <label class='control-label col-xs-1'><?= get_phrase('transaction_date'); ?></label>
                    <div class='col-xs-2'>
                        <!-- <input id="transaction_date" type='text' name='voucher_date'  class='form-control required' autocomplete="off" /> -->
                        <input class='form-control  required' name='voucher_date' type="text" value="<?= $voucher_header_info['voucher_date'] ?>" id="transaction_date" />
                    </div>

                    <label class='control-label col-xs-1'><?= get_phrase('voucher_number'); ?></label>
                    <div class='col-xs-2'>
                        <input type='text' onkeydown="return false" class='form-control required' name='voucher_number' id="voucher_number" value='<?= $voucher_header_info['voucher_number'] ?>' readonly />
                    </div>
                    ...
                    <div class='col-xs-2'>
                        <select class='form-control required' name='fk_voucher_type_id' id='voucher_type'>
                            <option value="<?= $voucher_header_info['fk_voucher_type_id']; ?>"><?= get_phrase($voucher_header_info['voucher_type_name']); ?></option>
...
<script>
    ...
    // Get office voucher date on load Added by Onduso on 2/06/2023

    $(document).ready(function() {

        let office_id = $('#office').val();

        computeCurrentTransactingDate(office_id);

        //Draw the voucher type dropdown
        ...
        $.get(url, function(response) {
            ...
            if (response_voucher_type.length > 0) {
                $.each(response_voucher_type, function(i, el) {
                    if (response_voucher_type[i].voucher_type_id != parseInt(saved_voucher_type_to_edit)) {
    ...
    function unitCostCell(value = '') {
        return "<td><input  name='voucher_detail_unit_cost[]' type='text' class='form-control required body-input number-fields unitcost' onclick='replaceValue(this);'  onchange='computeTotalCost(this);'  value='" + value + "' /></td>";
    }
    ...
    function saveVoucher(clicked_btn) {
        ...
        var voucher_number = $('#cheque_number').val();
        ...
        if (voucher_number != '') {
            ...
            var url = "<?= base_url(); ?>Voucher/get_duplicate_cheques_for_an_office/" + office_id + '/' + voucher_number + '/' + office_bank_id;
