 ...
 /**
   ...
   */
   ...
   public function toggle_list_select_columns(){
    ...
    $selected_results = $query_result['selected_results'];
    $total_records = $query_result['total_records'];
  
    // Implemeting resetting of options if a field is changed from to a select type
    $query_result['selected_results'] = $this->CI->grants->update_query_result_for_fields_changed_to_select_type($this->controller,$selected_results);
     ...
     */
     ...
     function _output($args){
      ...
      $load_data = isset($args[0]) && $args[0] == 'data' ? true : false;
      ...
      if($load_data){
        $this->CI->benchmark->mark('result_start');
        $result =$this->toggle_list_query_results()['selected_results'];
        $this->CI->benchmark->mark('result_end');

        $this->CI->benchmark->mark('fields_meta_data_start');
        $fields_meta_data = $this->CI->grants_model->fields_meta_data_type_and_name($table);
        $this->CI->benchmark->mark('fields_meta_data_end');
        
        $this->CI->benchmark->mark('total_records_start');
        $total_records =   19;// $this->toggle_list_query_results()['total_records'];
        $this->CI->benchmark->mark('total_records_end');

        // log_message('error', json_encode($this->CI->benchmark->elapsed_time('code_start', 'code_end')));
        // log_message('error', json_encode([
        //   'result' => $this->CI->benchmark->elapsed_time('result_start', 'result_end'),
        //   'fields_meta_data' => $this->CI->benchmark->elapsed_time('fields_meta_data_start', 'fields_meta_data_end'),
        //   'total_records' => $this->CI->benchmark->elapsed_time('total_records_start', 'total_records_end'),
        // ]));
