...
class User_account_activation_model extends MY_Model
{
    ...
    public function activate_new_user_account(int $user_activation_id): int
    {
        ...
        $this->read_db->where(['user_id'=>$fk_user_id]);
        ...
        $max_status_id = $this->general_model->get_max_approval_status_id('user', [], $account_system_id);
        $update_user['fk_status_id']= $max_status_id[0];

        // log_message('error', json_encode(compact('max_status_id','account_system_id','user_activation_id')));
