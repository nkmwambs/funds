...
class Language_model extends MY_Model
{
  ...
  function catch_language_phrase($handle, $translated_string){
    ...
    $user_language =  $language_obj->num_rows() > 0 ? $language_obj->row()->language_id : 1;

    $data['phrase'] = $handle;
    $data['phrase_translation'] = $translated_string;
    $data['fk_account_system_id'] = $account_system_id;
    $data['fk_language_id'] = $user_language;

    $this->read_db->where(array('phrase' => $handle, 'fk_account_system_id' => $account_system_id, 'fk_language_id' => $user_language));
