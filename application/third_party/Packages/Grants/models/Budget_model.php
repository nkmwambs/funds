...
private function list_budget_month_order($budget_id, $custom_year_start_date = ""){
  ...
  function check_if_all_budget_items_are_approved($budget_id){
    $return = true;
    $this->read_db->select(['fk_account_system_id', 'budget.fk_status_id as budget_status_id']);
    $this->read_db->join('budget','budget.fk_office_id=office.office_id');
    $this->read_db->where(['budget_id' => $budget_id]);
    $result_obj = $this->read_db->get('office');

    $account_system_id = $result_obj->row()->fk_account_system_id;
    $budget_status_id = $result_obj->row()->budget_status_id;
    $budget_item_max_states = $this->general_model->get_max_approval_status_id('budget', [], $account_system_id);

    if(!in_array($budget_status_id, $budget_item_max_states)){
      $budget_item_initial_status = $this->grants_model->initial_item_status('budget_item', $account_system_id);

      $this->read_db->select(['budget_item_id','fk_status_id']);
      $this->read_db->where(['fk_budget_id' => $budget_id]);
      $budget_items_obj = $this->read_db->get('budget_item');
      ...
      if($budget_items_obj->num_rows() > 0){
        ...
        $declined_item_status = $this->general_model->account_system_decline_states('budget_item', $account_system_id); // $this->general_model->decline_status($budget_item['budget_item_id'],$account_system_id);
