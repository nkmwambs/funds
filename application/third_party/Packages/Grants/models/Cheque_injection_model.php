...
class Cheque_injection_model extends MY_Model
{
    ...
    function update_injected_cheque_status($office_bank_id, $cheque_number){
        $is_injected_cheque_number = $this->is_injected_cheque_number($office_bank_id, $cheque_number);

        if($is_injected_cheque_number){
            $this->write_db->set('cheque_injection_is_active', 0);
            $this->write_db->where(array(
                "fk_office_bank_id" => $office_bank_id,
                'cheque_injection_number' => $cheque_number
            ));
            $this->write_db->update('cheque_injection');
            return true;
        }
        return false;
    }
    ...
    public function show_list_edit_action($record): bool {
        if($record['cheque_injection_is_active'] == 0){
            return false;
        }
        return true;
    }
