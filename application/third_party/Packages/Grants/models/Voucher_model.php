...
class Voucher_model extends MY_Model
{
    ...
    /**
     *get_vouchers_missing_voucher_details(): Returns a row of voucher information from voucher table [Main header table]
     * @author Livingstone Onduso: Dated 08-05-2023
     * @access public
     * @param int $office_id, string $reporting_month
     * @return array - returns one row of array
     */

    //  public function get_vouchers_missing_voucher_details(int $office_id, string $reporting_month):array
    //  {

    //     //Get the voucher dates
    //     $voucher_start_date= date('Y-m-01', strtotime($reporting_month));

    //     $voucher_end_date= date('Y-m-t', strtotime($reporting_month));

    //     //Inline SQL query
    //     $sql="SELECT voucher_id,voucher_date FROM voucher
    //     WHERE fk_office_id=$office_id AND voucher_date >='$voucher_start_date' AND voucher_date <='$voucher_end_date'
    //     AND voucher_id NOT IN (SELECT fk_voucher_id FROM voucher_detail)";

    //     //Sanitize the inline SQL with codeigniter function
    //     $missing_vouchers=$this->read_db->query($sql)->result_array();

    //     return $missing_vouchers;
     ...
     */
     ...
     public function is_voucher_missing_voucher_details(int $voucher_id):bool
     {

        //If no record of passed voucher_id means detail deleted
        $this->read_db->select(['fk_voucher_id']);

        $this->read_db->where(['fk_voucher_id'=>$voucher_id]);

        $vouchers_with_voucher_detail=$this->read_db->get('voucher_detail')->result_array();
  ...
  /**
   * get_secondary_role_ids(): returns records of secondary role ids for the logged in user
   * @author  Onduso 
   * @dated: 31/05/2024
   * @access public
   * @return array
   */
  public function get_secondary_role_ids():array{

    $this->read_db->select(['fk_role_id']);
    $this->read_db->join('role','role.role_id=role_user.fk_role_id');
    $this->read_db->where(['fk_user_id'=>$this->session->user_id]);
   
    $role_ids= $this->read_db->get('role_user')->result_array();

    $ids=[];

    foreach($role_ids as $role_id){

      $ids[]=$role_id['fk_role_id'];
    }
 ...
 /**
   * get_immediate_next_next_status(): returns records of secondary role ids for the logged in user
   * @author  Onduso 
   * @dated: 31/05/2024
   * @access public
   * @return int
   * @param int $status_id
   */
  public function get_immediate_next_next_status(int $status_id):int{

   
     //If $next_status<>final level
     $final_approval_status=$this->general_model->get_max_approval_status_id('voucher')[0];

     $immediate_status = $status_id;

    if((int)$final_approval_status!=$status_id){

            //Get next positive level for the $status_id
            $this->read_db->select(['status_approval_sequence']);

            $this->read_db->where(['status_id' => $status_id]);

            $next_status = $this->read_db->get('status')->result_array();

            $status_approval_sequence = ((int)$next_status[0]['status_approval_sequence']) + 1;

            //get the next immediate status
            $this->read_db->select(['status_id']);
            $this->read_db->where(['status_approval_sequence' => $status_approval_sequence, 'status_approval_direction' => 1]);
            $immediate_status_res = $this->read_db->get('status')->result_array();

            $immediate_status = $immediate_status_res[0]['status_id'];
  ...
  /**
   * get_original_voucher_id(): returns a voucher id value for a cancelled voucher
   ...
   * @param int $voucher_id
   */
  public function get_original_cancelled_voucher_id(int $voucher_id):int{

    $this->read_db->where(array('voucher_reversal_to' => $voucher_id));
    
    $voucher_id_row_obj=$this->read_db->get('voucher');
  
    if($voucher_id_row_obj->num_rows()>0){
     return $voucher_id_row_obj->row()->voucher_id;
    }
  ...
  /**
   * update_voucher_immediate_next_next_status(): updates the next_next immediate status for a caancelled voucher
   * @author  Livingstone Onduso 
   * @dated: 31/05/2024
   * @access public
   * @return void
   * @param int $original_voucher_id, array $next_status_data
 */
  public function update_voucher_immediate_next_next_status(int $original_voucher_id, array $next_status_data):void{
    
    $this->write_db->where(['voucher_id' => $original_voucher_id]);
   ...
   * @return 
   */
  public function insert_zero_amount_voucher(int $cheque_number, int $cheque_id, int $office_bank_id, $cnt)
  {
    ...
    $last_voucher_number_and_date = $this->get_office_last_voucher($office_id);
    $voided_chq_voucher_type = $this->voucher_type_model->get_hidden_voucher_type('VChq', $office['account_system_id']);// $this->voucher_type_model->get_hidden_voucher_type('bank', 'expense', 2, 1)->voucher_type_id;
    $accounts=$this->get_income_and_expense_for_account_system($office['account_system_id']);
    ...
    //Set voucher cleared month and cleared status
    $header['voucher_cleared'] =1;
    $header['voucher_cleared_month']=$last_voucher_number_and_date['voucher_date'];
    ...
    $detail['fk_project_allocation_id'] =$this->get_atleast_one_fcp_project_allocation($office_id);
   ...
   * @return  int
   */
  private function get_atleast_one_fcp_project_allocation(int $office_id):int{

    $this->read_db->select(['project_allocation_id']);

    $this->read_db->where(['fk_office_id'=>$office_id]);

    $project_allocation_id=$this->read_db->get('project_allocation');

    if($project_allocation_id->num_rows()>0){
        return $project_allocation_id->row()->project_allocation_id;
    }else{
        return 0;
    }
   ...
   * @return  array
   */
  private function get_income_and_expense_for_account_system(int $account_system_id):array{

    $accounts=[];

    $this->read_db->select(['income_account_id','expense_account_id']);
    ...
    $income_and_expense_acc=$this->read_db->get('income_account');

    if($income_and_expense_acc->num_rows()>0){
        $accounts['income_acc']=$income_and_expense_acc->row()->income_account_id;
        $accounts['expense_acc']=$income_and_expense_acc->row()->expense_account_id;
    }
   ...
   * @return int
   */
  public function revert_cancelled_cheque_and_related_voucher(int $voucher_id):int
  {
    ...
    $vouchers=$this->get_voucher_numbers_for_renumbering($voucher_id);
    ...
    foreach($vouchers as $voucher){
        ...
        $this->write_db->where(['voucher_id'=>$voucher['voucher_id']]);
   ...
   * @return array
   */
  private function get_voucher_numbers_for_renumbering(int $voucher_id):array{
   ...
   */
   $this->write_db->select(['voucher_date','fk_office_id']);

   $this->write_db->where(['voucher_id'=>$voucher_id]);

  $voucher_date_and_office= $this->write_db->get('voucher')->row();

   //Get all vouchers of the months
   $this->write_db->select(['voucher_id','voucher_number']);

   $this->write_db->where(['fk_office_id'=>$voucher_date_and_office->fk_office_id, 'voucher_date >=' => date('Y-m-01', strtotime($voucher_date_and_office->voucher_date)), 'voucher_date <=' => date('Y-m-t', strtotime($voucher_date_and_office->voucher_date))]);

   return $this->write_db->get('voucher')->result_array();
    ...
    public function total_cost_for_voucher_to_edit(int $voucher_id): float
    {
       ...
       // $total_cost = 0.00;
        ...
        $arr_result=$this->read_db->get('voucher_detail')->result_array();
    ...
    public function get_duplicate_cheques_for_an_office($office_id, $cheque_number, $office_bank_id, $hold_cheque_number_for_edit = 0, $has_eft)
    {
    ...
    public function post_approval_action_event(array $item){
        // Force the original voucher to take the next approval status of the reversal voucher
        $voucher_id = $item['post']['item_id'];
        // log_message('error',json_encode($item));

        $this->read_db->where(array('voucher_id' => $voucher_id));
        $this->read_db->or_where(array('voucher_reversal_to' => $voucher_id));
        $voucher_reversals_obj = $this->read_db->get('voucher');

        // log_message('error', json_encode($voucher_reversals_obj->result_array()));

        // if($voucher_reversals_obj->num_rows() > 0){
            if($voucher_reversals_obj->num_rows() > 1){
                // This is a reversal voucher
                $result = [];
                $voucher_reversals = $voucher_reversals_obj->result_array();
                ...
                $this->write_db->where(array('voucher_id' => $result['original']['voucher_id']));
                $this->write_db->update('voucher', ['fk_status_id' => $item['post']['next_status']]);
            }
        // }

        // Fully approved cheque cancellation
       
        // $this->read_db->where(['fk_voucher_id']);
        // $this->read_db->join('office','office.office_id=voucher.fk_office_id');
        // $voucher_obj = $this->read_db->get('voucher')->row();

        // $this->read_db->where(['fk_voucher_id']);
        // $cancel_cheque_obj = $this->read_db->get('cancel_cheque');

        // $max_voucher_status_ids = $this->general_model->get_max_approval_status_id('voucher', [], $voucher_obj->accounting_system_id);
        // // $next_status = $item['post']['next_status'];

        // if(in_array($voucher_obj->fk_status_id, $max_voucher_status_ids) && $cancel_cheque_obj->num_rows() > 0){
        //     // Update the cheque_cancallation to fully approved
        //     $this->write_db->where(['fk_voucher_id']);
        //     $this->write_db->update('cancel_cheque',['fk_status_id' => $max_voucher_status_ids[0]]);
        // }
