...
class Financial_report_model extends MY_Model
{
    ...
    //   log_message('error', json_encode($opening_bank_balance));
     ...
     $bank_to_bank_contra_contributions = $this->bank_to_bank_contra_contributions($office_bank_ids, $reporting_month);
    ...
    function month_income_account_receipts($office_ids, $start_date_of_month, $project_ids = [], $office_bank_ids = [])
    {
        ...
        $bank_to_bank_contra_contributions = $this->bank_to_bank_contra_contributions($office_bank_ids, $start_date_of_month);
        ...
        foreach ($income_accounts as $income_account) {
            ...
            if(isset($this->bank_to_bank_contra_contributions($office_bank_ids, $start_date_of_month)[$income_account['income_account_id']])){
    ...
    function income_accounts($office_ids, $project_ids = [], $office_bank_ids = [])
    {
        ...
        $this->read_db->group_by(array('income_account_id'));
        $result = $this->read_db->select(array('income_account_id', 'income_account_name','income_account_is_budgeted'))->get('income_account')->result_array();
    ...
    function budget_to_date_by_expense_account($office_ids, $reporting_month, $project_ids = [], $office_bank_ids = []){
        ...
        $this->read_db->group_by('month_number,expense_account.expense_account_id');
        $this->read_db->where_in('budget.fk_office_id', $office_ids);
    ...
    function list_cleared_effects($office_ids, $reporting_month, $transaction_type, $contra_type, $voucher_type_account_code, $project_ids = [], $office_bank_ids = [])
    {
        ...
        $this->read_db->group_start();
          ...
          $this->read_db->or_where(['voucher_type_is_hidden' => 2]);//Voided cheque
        $this->read_db->group_end();
    ...
    function check_if_financial_report_is_submitted($office_ids, $reporting_month)
    {
        ...
        //if (is_array($office_ids) && count($office_ids) == 1) {
            ...
            $this->read_db->select(['financial_report_is_submitted']);
            $this->read_db->where([ 'financial_report_month'=>date('Y-m-01', strtotime($reporting_month)),'fk_office_id' => $office_ids[0]]);
            $financial_report_is_submitted_obj=$this->read_db->get('financial_report');

            //log_message('error',$financial_report_is_submitted_obj->num_rows());
            ...
            if ($financial_report_is_submitted_obj->num_rows() > 0) {
                ...
                if ($financial_report_is_submitted_obj->row()->financial_report_is_submitted==1) {
