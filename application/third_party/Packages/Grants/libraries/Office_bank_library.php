 ...
 */
 ...
 class Office_bank_library extends Grants
 {
  ...
  private function active_cheque_book_warning($hash_office_bank_id){
    $message = '';
    $office_bank_id = hash_id($hash_office_bank_id,'decode');
    $this->CI->load->model('cheque_book_model');
    $leaves = $this->CI->cheque_book_model->get_remaining_unused_cheque_leaves($office_bank_id);
    if(!empty($leaves)){
  ...
  function page_position(){
    ...
    if($this->CI->action == 'view' || $this->CI->action == 'edit'){
      ...
      $message .= $this->active_cheque_book_warning($this->CI->id);
      if($message != ""){
