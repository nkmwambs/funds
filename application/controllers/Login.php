...
class Login extends CI_Controller
{
    ...
    function validate_login(string $email = '', string $password = '', bool $is_user_switch = false): string
    {
        ...
        if ($md5_old_user_password->num_rows() > 0 && !$is_user_switch) {
            ...
            //echo json_encode($this->create_user_session($row, $is_user_switch));
        ...
        if ($query->row()->user_first_time_login == 0 && !$is_user_switch) {
            ...
            return 'success';
        } elseif ($query->num_rows() > 0 && ($maintenance_mode == 0 || ($maintenance_mode == 1 && $query->row()->user_is_system_admin == 1))) {
            $row = $query->row();
    ...
    //  * @param int $context_definition_id
    //  */
    // public function get_user_departments(int $context_definition_id): void
    // {

    //     $departments = $this->login_model->get_user_departments($context_definition_id);
     ...
     */
    // public function get_user_roles(int $context_definition_id, int $account_system_id): void
    // {

    //     $roles = $this->login_model->get_user_roles($context_definition_id, $account_system_id);

    //     echo json_encode($roles);
    // }

    /**
     * get_user_designations(): returns designation based on context definiation
     * @author Onduso 
     * @access public 
     * @return void
     * @param int $context_definition_id
     */
    // public function get_user_designations(int $context_definition_id): void
    // {

    //     $designations = $this->login_model->get_user_designations($context_definition_id);
