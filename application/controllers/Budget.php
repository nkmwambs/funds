...
class Budget extends MY_Controller
{
    ...
    foreach ($budget_item_details as $row) {
      ...
      $result_grid[$row->income_account_id]['budget_items'][$row->expense_account_id]['expense_items'][$row->budget_item_id] =
        [
          ...
          'budget_item_revisions' => json_decode($row->budget_item_revisions),
          'objectives' => json_decode($row->budget_item_objective)
        ];
    }
  ...
  function result($id = '')
  {
    ...
    if ($this->action == 'view') {
      ...
      if(!$this->budget_model->check_if_all_budget_items_are_approved($budget_id)){
        $result['action_button_disabled'] = true;
