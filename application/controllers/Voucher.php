...
class Voucher extends MY_Controller
{
  ...
  public function revert_cancelled_cheque_and_related_voucher(int $voucher_id){

    $save_status=$this->voucher_model->revert_cancelled_cheque_and_related_voucher($voucher_id);
  ...
  public function show_list()
  {
    ...
    foreach ($vouchers as $voucher) {
      ...
      //  $voucher_missing_details=$this->voucher_model->is_voucher_missing_voucher_details($voucher_id);
      $action = '';
      
      if (is_array($month_cancelled_vouchers) && !in_array($voucher_id, $month_cancelled_vouchers)) {
        $action = approval_action_button($this->controller, $item_status, $voucher_id, $voucher_status, $item_initial_item_status_id, $item_max_approval_status_ids,false, true,'', $is_voided_chq);
      }
...
/**
   * update_item_status(): updates the voucher status accorrdingly
   ...
   * @param int $item
   */
  public function update_item_status($item):void
  {
    ...
    if($voucher_missing_details){

      //Get the hash_id and redirect the voucher
      $hashed_voucher_id=hash_id($voucher_id, 'encode');

      $path=base_url().'voucher/edit/'.$hashed_voucher_id;

      log_message('error', json_encode('Just that you know! vouchers are still missing Voucher Details'));

      echo "<script>window.location.href = '$path';</script>";

    }else{

      //Approving Non cancelled vouchers
      parent::update_item_status($item);

      //Approving a cancelled voucher piece of code
      //Update status of the original voucher after reversal
      $original_voucher_id=$this->voucher_model->get_original_cancelled_voucher_id($voucher_id);

      if ($original_voucher_id > 0) {

        $status_id_from_input = $this->input->post('next_status');

        $next_status_data['fk_status_id']= $status_id_from_input ;

        //Get the secondary actor on this next status. if so get the immediate next. 
        $secondary_role_id=$this->voucher_model->get_secondary_role_ids();

        if(!empty($secondary_role_id)){

          $status_id=(int)$status_id_from_input;

          //Get the next next immediate status
          $immedidate_status_after_next_status=$this->voucher_model->get_immediate_next_next_status($status_id);

          $next_status_data['fk_status_id']=$immedidate_status_after_next_status;

        }
        
        //Update db with correct status if voucher was cancelled
        $this->voucher_model->update_voucher_immediate_next_next_status($original_voucher_id,$next_status_data);
  ...
  function get_duplicate_cheques_for_an_office($office_id = 0, $cheque_number = '', $office_bank_id = 0, $hold_cheque_number_for_edit, $has_eft = '')
  {
  ...
  public function check_cheque_validity(): void
  {
    ...
    $leaves = $this->cancel_cheque_model->get_valid_cheques($office_bank_id);
    // log_message('error', json_encode($leaves));
    //Added by Onduso on 7/6/2023 to be used by Edit voucher function on used checks
    $chq_to_edit_arr = [];
  ...
  function insert_new_voucher()
  {
    ...
    $track = $this->grants_model->generate_item_track_number_and_name('voucher');
    ...
    $office_bank_id = $this->get_office_bank_id_to_post($office_id);
    ...
    $voucher_cheque_number =$this->input->post('voucher_cheque_number') == null ? 0 : $this->input->post('voucher_cheque_number');
    $header['voucher_cheque_number'] = $voucher_cheque_number;
    $this->load->model('cheque_injection_model');
    $this->cheque_injection_model->update_injected_cheque_status($office_bank_id, $voucher_cheque_number);
    ...
    if($header['fk_cheque_book_id'] != NULL){
      $this->write_db->where(array('cheque_book_id' => $header['fk_cheque_book_id']));
      $this->write_db->update('cheque_book', ['cheque_book_is_used' => 1]);
    }
