...
class Journal extends MY_Controller
{
  ...
  function reverse_voucher($voucher_id,$is_reuse_cheque_transaction = 1, $reusing_and_cancel_eft_or_chq='',$journal_month=''){
    ...
    if($count_of_reuse < $this->config->item('cheque_cancel_and_resuse_limit') || $this->session->system_admin){
         ...
         // log_message('error', json_encode($journal_month));
          ...
          $insert_voucher = $this->voucher_model->insert_voucher_reversal_record($voucher,$is_reuse_cheque_transaction,$journal_month);
